...
class LocNDF(LightningModule):
    ...
    def training_step(self, batch: dict, batch_idx):
        ...
        radius1 = self.compute_gradient_radius(batch['inter'])
        ...
        return loss
    def compute_gradient_radius(self, position: torch.Tensor):
        position.requires_grad_(True)
        val = self.forward(position)
        #print("y_val :",np.shape(val))
        #print("x_position ", np.shape(position[:,:,:,0:3]))
        #print("x_position ", (position[:,:,:,0:3]))
        #position=position[:,:,:,0:3].requires_grad_(True)
        grad = utils.compute_gradient(val, position)
        #print('gradies_grad ',grad)
        d2y_dx2 = utils.compute_gradient(grad, position)
        dy_dx=grad
        #dist = torch.einsum(
        #        '...n,...n->...',(dy_dx.detach()), d2y_dx2.detach())
        #dist=torch.einsum('ijkl,ijkl->', dy_dx, d2y_dx2)
        #print((dist))
        #radius = (torch.abs(( dy_dx)**3)) / torch.sqrt(((torch.abs(d2y_dx2)**2)*(torch.abs(dy_dx))**2)-(dist))
        radius = ((1 + grad**2)**1.5) / torch.abs(d2y_dx2)
        return radius
    def compute_gradient_dists(self, position: torch.Tensor):
        position.requires_grad_(True)
        val = self.forward(position)
        #print("y_val :",val)
        #print("x_position ", position)
        grad = utils.compute_gradient(val, position)
        #print('gradies_grad ',grad)
